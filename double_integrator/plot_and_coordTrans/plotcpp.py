...
#    plt.plot(SDIbd[0:5,0],SDIbd[0:5,i+1],color = 'b')
#    plt.plot(SDIbd[0:5,0],SDIbd[0:5,i+num_state+1],color = 'b')
...
SDI_orig=np.genfromtxt('SDI_orig.csv', delimiter=',')
name = ['x (m)','y (m)','$\\phi$', '$\\delta$']
t = DIcstz[:,0];
for i in range(5):
    F=plt.figure(i) 
    for s in range(64):
    ...
    plt.plot(t[0:6],SDI_orig[0:6,i],color = 'b')
    plt.plot(t[0:6],SDI_orig[0:6,i+5],color = 'b')
