...
typedef struct {// define variables that are uncertain: initial conditions and/or parameters
    IA uncertainties[num_uncertainties], stateI[num_state_redundant], referenceInput[num_reference_input], natural_bound[num_state_redundant];
} *UserData;



int DI_constraints(double X0[2*num_state_redundant], double Uncertainty[2*num_uncertain_parameter], memberInfoDI_constraints Input){


    int num_state_for_odesolver=2*num_state_redundant; //ODE state solution +   desired state solution from open loop ODE with given control input
    //  extern double X0;
    N_Vector x, abstol;
    ...
    ofstream outfile;
    if(File_Output_DI){
        outfile.open ("StatesBD_DI_CST.csv");
    ...
    for (int i = 0; i < 2*num_state_redundant  ; i++) {
        Ith(x,i+1) = X0[i];
        (*data).natural_bound[i].l(Input.NaturBD[i]);
        (*data).natural_bound[i].u(Input.NaturBD[i+num_state_redundant]);
    }
    ...
    for(int pctime=0;pctime<num_piecewise;pctime++) {
        ...
        while (1) {
            ...
            for(int j=0;j<num_state_redundant;j++){
                (*data).stateI[j].l(NV_Ith_S(x,j));
                (*data).stateI[j].u(NV_Ith_S(x,j+num_state_redundant));
            }
            // natural bound
            for(int j=0;j<num_state_redundant;j++){
                (*data).stateI[j].l( min(max((*data).stateI[j].l(),Input.NaturBD[j]),(*data).stateI[j].u()));
                (*data).stateI[j].u( max(min((*data).stateI[j].u(),Input.NaturBD[j+num_state_redundant]),(*data).stateI[j].l()));
            }
            ...
            for(int j=0;j<num_state_redundant;j++){
                ...
                NV_Ith_S(x,j+num_state_redundant) = (*data).stateI[j].u();
            }
...
static int freal (realtype t, N_Vector x, N_Vector xdot, void *user_data) {
    ...
    //natural bounds
    //for (int i = 0; i < num_state_redundant; i++){
    //    min(NV_Ith_S(x,i),);
...
//        }
        ...
        NV_Ith_S(xdot, i+num_state_redundant)=fxu.u();
