...
typedef struct {// define variables that are uncertain: initial conditions and/or parameters
    IA uncertainties[num_uncertainties], stateI[num_state_variable], referenceInput[num_reference_input];
} *UserData;


int SDI(double X0[2 * num_state_variable], double Uncertainty[2 * num_uncertain_parameter], memberInfoSDI Input) {


    int num_state_for_odesolver = 2 *
                                  num_state_variable; //ODE state solution +   desired state solution from open loop ODE with given control input
    ...
    for (int i = 0; i < 2 * num_state_variable; i++) {
        Ith(x, i + 1) = X0[i];
    }
...
static int freal(realtype t, N_Vector x, N_Vector xdot, void *user_data) {
    ...
    //IA P[num_uncertain_parameter];

    for (int j = 0; j < num_state_variable; j++) {
        (*data).stateI[j].l(NV_Ith_S(x, j));
        (*data).stateI[j].u(NV_Ith_S(x, j + num_state_variable));
    }
    ...
    for (int i = 0; i < num_state_variable; i++) { //loop from fi=1 to fi=n
        ...
        (*data).stateI[i].u(NV_Ith_S(x, i + num_state_variable));

        //flat the lower bound to upper bound
        (*data).stateI[i].l(NV_Ith_S(x, i + num_state_variable));
        fxu = rhsI(t, (*data).stateI, (*data).uncertainties, (*data).referenceInput, i);
        (*data).stateI[i].l(NV_Ith_S(x, i));

        NV_Ith_S(xdot, i) = fxl.l();
        NV_Ith_S(xdot, i + num_state_variable) = fxu.u();
